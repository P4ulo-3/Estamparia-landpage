...
export default function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-end gap-3">
      {/* Tooltip */}
      {showTooltip && (
        <div className="relative bg-card border border-border rounded-xl p-3 shadow-xl max-w-[200px] animate-fade-in-up">
          <button
            onClick={() => setShowTooltip(false)}
            className="absolute -top-2 -right-2 w-5 h-5 bg-muted rounded-full flex items-center justify-center"
            aria-label="Fechar tooltip"
          >
            <X className="w-3 h-3 text-muted-foreground" />
          </button>
          <p className="font-sans text-xs text-card-foreground">
